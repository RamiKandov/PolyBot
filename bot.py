...
class YoutubeBot(Bot):
    def _message_handler(self, update, context):
        to_quote = True
        self.send_text(update, f'Hi Your original message: {update.message.text}', quote=to_quote)
        file_path = str(utils.search_download_youtube_video(update.message.text))
        context.bot.send_video(update, context, file_path)
'''
if __name__ == '__main__':
    ...
    my_bot = Bot(_token)
    my_bot.start()
    #my_bot2 = QuoteBot(_token)
    #my_bot2.start()
    #my_bot3 = YoutubeBot(_token)
    #my_bot3.start()
